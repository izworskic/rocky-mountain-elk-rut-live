...
page = page.replace('.metric strong{display:block;font-size:.97rem;margin-top:3px}', `.metric strong{display:block;font-size:.97rem;margin-top:3px}
...
page = page.replace(`<div class="metric-row">
...
page = page.replace(`<div class="arrival-time" id="arrival-time">--</div>
      <p id="arrival-copy">The recommended arrival builds in setup time before the crepuscular activity window.</p>`, `<div class="arrival-time" id="arrival-time">--</div>
...
page = page.replace(`<p id="access-detail" style="margin-top:12px">Moraine Park has different timed-entry rules from most of the park.</p>`, `<p id="access-detail" style="margin-top:12px">Moraine Park has different timed-entry rules from most of the park.</p>
...
page = page.replace('    <p>${esc(s.note)}</p>\n    <div class="access">', '    <p>${esc(s.note)}</p>\n    <div class="spot-actions"><a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${s.lat},${s.lon}`)}" target="_blank" rel="noopener">Open map ↗</a>${s.id === "harbison-meadow" ? `<a href="https://www.nps.gov/romo/learn/photosmultimedia/webcams.htm" target="_blank" rel="noopener">Check Kawuneeche webcam ↗</a>` : ""}</div>\n    <div class="access">');

page = page.replace(`function renderPrimary(data) {
  const w = data.primary;`, `let countdownTimer = null;
...
page = page.replace(`  $("primary-verdict").textContent =
    ...
    "The rut is active seasonally, but this window has meaningful tradeoffs.";`, `  const hazard = w.weather?.[w.bestZone]?.hazard;
...
page = page.replace(`  $("arrival-time").textContent = w.arrivalLabel;

  const moraine`, `  $("arrival-time").textContent = w.arrivalLabel;
  ...
  $("next-choice-detail").textContent = same ? "The next window is also the best near-term choice." : `${next.startLabel}–${next.endLabel} · ${w.score - next.score > 0 ? `${w.score - next.score} points below the best` : "Comparable to the best"}`;
